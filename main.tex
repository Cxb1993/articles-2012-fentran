...
\noindent
\textsl{Departamento de Engenharia Mecânica
\\
Programa de Pós-Graduação em Engenharia Mecânica -- PPG-EM
\\
\noindent Universidade do Estado do Rio de Janeiro -- UERJ}
\\
...
\'oleo escoa em codinções de alta pressão e temperatura, formando bolhas
de g\'as. O conceito de fase, atrav\'es dos princ\'ipios de
termodin\^amica cl\'assica, \'e descrito como o estado macrosc\'opico da
mat\'eria no qual sua estrutura f\'isica e sua composi\c c\~ao qu\'imica
s\~ao homog\^eneas. Na presen\c ca de duas ou mais fases, origina-se o
que chamamos de escoamentos multif\'asicos, onde o termo bif\'asico se
...
fen\^onemno bif\'asico e que muitas vezes nos passa desapercebida. No
primeiro exemplo da Fig.~(\ref{fig:surfTension}a) podemos observar a
...
contamina\c c\~ao dos fluidos. O segundo exemplo da 
...
abaulamento do l\'iquido junto \`a parede do copo. No terceiro
exemplo da Fig.~(\ref{fig:surfTension}c) e no quarto exemplo
da Fig.~(\ref{fig:surfTension}d) s\~ao ilustradas duas gotas em repouso. Na
...
\begin{figure}[h!]
	\begin{center}
		...
		{\includegraphics[angle=00, scale=0.38]{surfTension1.eps}
		\hspace{0.6cm}}
		\subfloat[capilaridade]
		{\includegraphics[angle=00, scale=0.38]{surfTension3.eps}}\\
		\subfloat[superf\'icie hidrof\'ilica]
		{\includegraphics[angle=00, scale=0.38]{surfTension2a.eps}
		\hspace{0.6cm}}
		\subfloat[superf\'icie hidrof\'obica]
		{\includegraphics[angle=00, scale=0.38]{surfTension2b.eps}}
	\end{center}
	\caption[Exemplos de sistemas bif\'asicos]{Exemplos de sistemas
	bif\'asicos encontrados na natureza. (a) Esfera em repouso. Neste
	sistema a resultante das for\c cas \'e zero pois o peso da esfera
	\'e correspondido pelo efeito da tens\~ao superficial do l\'iquido.
	(b) Efeitos de capilaridade fazem o l\'iquido subir sobre as paredes
	do tubo. (c) Em uma superf\'icie hidrof\'ilica, a gota tende a se
	espalhar, enquanto em uma (d) superf\'icie hidrof\'obica, a gota
	tende a ficar concentrada em uma \'area de contato menor.}
...
f\'isicos \'e necess\'aria em diversos campos da ci\^encia. Trabalhos
...
modelo de um fluido, as equa\c c\~oes que modelam o
...
para escoamentos monof\'asicos se encontra no Vol.~(1) deste livro,
...
de um fluido. Considere a seguinte equaç\~ao:
...
respectivamente. No entanto, no modelo de um fluido, estes
...
Neste caso, a nota\c c\~ao do lado esquerdo da Eq.~(\ref{eq:qm16}) \'e
...
que no segundo o referencial se movimenta com a velocidade do fluido.
Estes dois referenciais ainda podem ser descritos em uma forma \'unica e
generalizada, conhecida como referencial \emph{euleriano-lagrangiano}
arbitr\'ario onde este pode n\~ao estar nem fixo, nem em movimento e sim
em com uma velocidade arbitr\'aria. A defini\c c\~ao detalhada deste
referencial n\~ao se encontra no escopo do presente cap\'itulo. Para o
leitor interessado, recomenda-se: \cite{hughes1981} e \cite{donea1982}. 

\subsection{Adimensionaliza\c c\~ao das Equa\c c\~oes de Governo}

Esta se\c c\~ao descreve a adimensionaliza\c c\~ao das equa\c c\~oes de
...
equa\c c\~ao. Duas equa\c c\~oes distintas ser\~ao apresentadas: a
primeira \'e comumente usada quando a velocidade e a dimens\~ao s\~ao
par\^ametros conhecidos do problema. A segunda \'e geralmente empregada
para o caso de escoamentos regidos pelo campo gravitacional, sem condi\c
c\~ao de entrada de fluido. Neste caso a gravidade deve ser considerada
como par\^ametro conhecido, com isso a velocidade \'e um resultado e
n\~ao uma condi\c c\~ao preestabelecida. 
...
\begin{align}
	...
	p
	&=
	\rho_0 U^2 p^* & \frac{\partial}{\partial t}
	&=
...
do Vol.~(1) deste livro, o processo de adimensionaliza\c c\~ao das
equa\c c\~oes de governo para escoamento multif\'asico incompress\'ivel
chega a:
...
n\'umeros de \emph{Reynolds} ($Re=\rho_0 U L/\mu$) representa a raz\~ao
das for\c cas inercial e viscosa, o n\'umero de \emph{Froude}
($Fr=U/\sqrt{g_0 L}$) representa a raz\~ao das for\c cas inercial e
gravitacional e, finalmente, o n\'umero de \emph{Weber} ($We=\rho_0 U^2
L/\sigma_0$) representa a raz\~ao das for\c cas inercial e de tens\~ao
superficial.
...
condi\c c\~ao e n\~ao \'e imposta diretamente no sistema, um par\^ametro
de adimensionaliza\c c\~ao diferente deve ser usado no lugar. Como
exemplo pode se citar o caso de uma bolha de g\'as em ascens\~ao em uma
coluna de l\'iquido im\'ovel. Neste sistema, o campo de velocidades \'e
o resultado, com isso n\~ao pode ser usado como referencial. Ent\~ao, a
velocidade $\vvet$ \'e comumente adimensionalizada por $\sqrt{g_0 D}$ e o
comprimento caracter\'istico $L$ pelo di\^ametro da bolha $D$.
Substituindo estes par\^ametros na Eq.~(\ref{eq:qmFinal}), a equa\c
c\~ao final de conserva\c c\~ao de quantidade de movimento se escreve
como:
...
\noindent onde o n\'umero de \emph{Archimedes} ($Ar=g_0 L^3
\rho^2/\mu^2$) representa a raz\~ao das for\c cas inercial e
gravitacional, enquanto que o n\'umero de \emph{E\"otv\"os} ($Eo=\rho
g_0 L^2/\sigma_0$) representa a raz\~ao das for\c cas de gravidade e
tens\~ao superficial. O n\'umero de \emph{Morton} ($Mo=g_0 \mu_0^4
\rho_0/ \rho_0^2 \sigma_0^3$) \'e tamb\'em utilizado para caracterizar a
forma de bolhas e gotas em problemas multif\'asicos, apesar de n\~ao
aparecer explicitamente nas equa\'c\~oes de escoamento.

\section{Abordagem Num\'erica}

Em escoamentos bif\'asicos, a modelagem discreta da interface \'e o
par\^ametro mais importante para se obter precis\~ao nos c\'alculos.
Encontram-se na literatura duas abordagens cl\'assicas para representa\c
c\~ao da interface entre dois fluidos: \emph{euleriana} e
\emph{lagrangiana}. Estas duas abordagens est\~ao fortemente ligadas \`a
representa\c c\~ao do escoamento. Figura~(\ref{fig:interface})
representa um esquema bidimensional das descri\c c\~oes de interface nos
dois modos acima citados. 

Na descri\c c\~ao \emph{euleriana}, a interface n\~ao \'e representada
explicitamente, ao contr\'ario ela \'e definida atrav\'es de uma fun\c c\~ao de
identifica\c c\~ao de fases e movimentada atrav\'es de uma equa\c c\~ao
...
interface n\~ao divide nenhum elemento. Adicionalmente \`a
esta abordagem, a interface \'e representada sem qualquer difus\~ao, ou
seja, esta \'e definida com espessura zero. O movimento da interface \'e feito
...
\begin{figure}[ht!]
		...
		\includegraphics[scale=0.26]{interface1.eps}}
		\subfloat[]{\label{fig:interface2}
		\includegraphics[scale=0.26]{interface2.eps}}
...
\begin{figure}[ht!]
	\begin{center}
		...
		\includegraphics[scale=0.33]{vof.eps}}
		\subfloat[]{\label{fig:vof2}
		\includegraphics[scale=0.37]{vofzoom.eps}}
...
adimensionais \emph{E\"otv\"os} e \emph{Morton} foi testadas e
comparada a resultados experimentais, com isso provando a capacidade do
...
equa\c c\~oes que descrevem o movimento do fluido. A coalesc\^encia de bolhas e a bolha em
...
\begin{figure}[ht!]
	\begin{center}
		...
		\includegraphics[scale=0.27]{distance.eps}}
		\hspace{0.5cm}
		\subfloat[]{\label{fig:level-set}
		\includegraphics[scale=0.27]{level-set.eps}}
	\end{center}
...
esquema do tipo \emph{upwind}/proje\c c\~ao para o c\'alculo da solu\c
c\~ao das equa\c c\~oes de Navier-Stokes para fluidos imisc\'iveis. Foi
mostrado que a conserva\c c\~ao de massa \'e apenas garantida se a fun\c
c\~ao de Level-Set \'e reinicializada preferencialmente em todo passo de
tempo. Eles realizaram significativos testes com bolhas e gotas para
valida\c c\~ao desta nova metodologia. Adicionalmente,
\cite{sussman1997} apresentam sua vers\~ao extendida para o caso
tridimensional de superf\'icie livre. Novamente, v\'arios testes foram
realizados com sucesso para valida\c c\~ao desta nova extens\~ao
atrav\'es de importantes casos, tais quais gota est\'atica e gota
oscilante em gravidade zero.
...
escoamento de \emph{Poiseuille}, instabilidade de Rayleigh-Taylor e
ruptura de barragem. Todos os testes apresentaram boa concord\^ancia
com a literatura.
...
Ao contr\'ario dos m\'etodos \emph{eulerianos} Level-Set e VOF, os
m\'etodos \emph{lagrangianos} \emph{volume-tracking} e
\emph{front-tracking} s\~ao os que mais se destacam na literatura. O
primeiro utiliza part\'iculas marcadoras para reconstru\c c\~ao da
interface, combinando precis\~ao e rigor com relativamente baixo custo
de investimento (\cite{harlow1965},\cite{amsden1970}). O segundo,
...
com o escoamento calculado no referencial \emph{euleriano}. Tal descri\c
c\~ao fornece uma representa\c c\~ao definida da interface com alta
...
(descri\c c\~ao \emph{euleriana}), a malha de interface se move com a
...
Para iniciarmos a introdu\c c\~ao \`as t\'ecnicas experimentais, passamos
...
O t\'itulo de vapor ($x$) \'e definido pela raz\~ao da vaz\~ao
...
O fluxo de massa ($G$) \'e definido como a vaz\~ao m\'assica total
...
A fra\c c\~ao de vazios $\epsilon$ \'e um dos par\^ametros mais
importantes para caracteriza\c c\~ao de escoamentos multif\'asicos. Este
tamb\'em \'e de grande import\^ancia para a determina\c c\~ao de outros
par\^ametros, tais como viscosidade e densidade em escoamentos
multif\'asicos, obten\c c\~ao de velocidade m\'edia relativa e,
tamb\'em, como ferramenta fundamental em modelos de previs\~ao de
...
em um ponto ou pequeno volume quando feita experimentalmente. \'E
definida com $\epsilon_{local} = 0$ quando l\'iquido est\'a presente ou 
$\epsilon_{local} = 1$ quando vapor est\'a presente. Tipicamente \'e
realizada uma m\'edia temporal local para determin\c c\~ao da fra\c
c\~ao de vapor, sendo definida pela seguinte express\~ao:
...
\noindent onde $P_k(r,t)$ \'e uma fun\c c\~ao indicadora da presen\c ca
de vapor no ponto em fun\c c\~ao do tempo $t$.

A fra\c c\~ao de vazios cordal $\epsilon_{local}$ \'e tipicamente medida
...
luz deste feixe no outro lado do tubo, \'e poss\'ivel medir o
comprimento da fase gasosa. A vaz\~ao de vazios local \'e ent\~ao
determinada por: 

\begin{equation}
	\epsilon_{local} = \frac{L_G}{L_G+L_L}
\end{equation}
...
\begin{figure}[h!]
	\begin{center}
		\subfloat[Bolhas]
		{\includegraphics[angle=00, scale=0.25]{v_bubbly.eps}
		\hspace{0.5cm}}
		\subfloat[Golfada]
		{\includegraphics[angle=00, scale=0.25]{v_slug.eps}
		\hspace{0.5cm}}
		\subfloat[Batido]
		{\includegraphics[angle=00, scale=0.25]{v_churn.eps}
		\hspace{0.5cm}}
		\subfloat[Pouco anular]
		{\includegraphics[angle=00, scale=0.25]{v_wispy-annular.eps}
		\hspace{0.5cm}}
		\subfloat[Anular]
		{\includegraphics[angle=00, scale=0.25]{v_annular.eps}}
	\end{center}
...
\begin{enumerate}
	...
	pouco anular, as gotas est\~ao separadas ao inv\'es de
	aglomeradas.
\end{enumerate}

\subsection{Escoamentos horizontais}

Os padr\~oes de escoamento comumente encontrados em tubula\c c\~oes 
horizontais e inclinados s\~ao complexos devido \`a sua
...
% 		\hspace{0.8cm}}
		...
		{\includegraphics[angle=00, scale=0.22]{h_bubbly.eps}
 		\hspace{0.2cm}}
		\subfloat[Bolhas]
		{\includegraphics[angle=00, scale=0.22]{h_plug.eps}}\\
		\subfloat[Golfadas]
		{\includegraphics[angle=00, scale=0.22]{h_slug.eps}
 		\hspace{0.2cm}}
		\subfloat[Estratificado]
		{\includegraphics[angle=00, scale=0.22]{h_stratified.eps}}\\
		\subfloat[Estratificado ondulado]
		{\includegraphics[angle=00, scale=0.22]{h_wavy.eps}
 		\hspace{0.2cm}}
		\subfloat[Anular]
		{\includegraphics[angle=00, scale=0.22]{h_annular.eps}}
	\end{center}
...
\begin{enumerate}
	...
	de toda a circunfer\^encia, por\'em ser\'a mais espesso na base do
	tubo.
\end{enumerate}
...
por\'em sua descri\c c\~ao \'e feita atrav\'es de gr\'aficos bi-dimensionais. Na
...
canais verticais baseado em (\cite{hewitt1969}). Neste mapa pode-se
observar que cada padr\~ao de escoamento ocupa uma \'area no gr\'afico e
as linhas tracejadas delimitam a transi\c c\~ao.  Como pode-se observar
tamb\'em, estas mesmas linhas n\~ao ocupam todo o limite do gr\'afico, o
que sugere a falta de dados experimentais para descrever as transi\c
c\~oes em toda a escala. \'E importante notar que estes mapas devem ser
...
para escoamento horizontal baseado em (\cite{baker1954}). Este mapa \'e
largamente utilizado na ind\'ustria petroqu\'imica. No mapa pode-se
identificar os padr\~oes de escoamento em diferentes velocidades
m\'assicas superficiais de l\'iquido $G_L$ e de g\'as $G_G$. 
...
\begin{figure}[ht!]
	...
	\\ (b) \\
	\caption[Mapas de padr\~oes de escoamento bif\'asico.]
	{Mapas de
	padr\~oes de escoamento bif\'asico representado por fluxo de massa
	$G$ de l\'iquido $G_L$ e de g\'as $G_G$. As linhas cont\'inuas e
	tracejadas representam a transi\c c\~ao dos padr\~oes de escoamento.
	(a) Escoamento vertical para ar/\'agua baseado em
	(\cite{hewitt1969}). (b) Escoamento horizontal para ar/\'agua
	baseado em (\cite{baker1954}).}
	\label{fig:map} 
\end{figure}
